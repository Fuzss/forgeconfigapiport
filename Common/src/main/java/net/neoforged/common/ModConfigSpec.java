...
public class ModConfigSpec extends UnmodifiableConfigWrapper<UnmodifiableConfig> implements IConfigSpec<ModConfigSpec>//TODO: Remove extends and pipe everything through getSpec/getValues?
{
    ...
    public synchronized boolean isCorrect(CommentedConfig config) {
        ...
        return this.correct(this.config, config, null, parentPath, Collections.unmodifiableList( parentPath ), (a, b, c, d) -> {}, null, true) == 0;
    }
    ...
    public synchronized int correct(CommentedConfig config, CorrectionListener listener, CorrectionListener commentListener) {
        ...
        try {
            ...
            ret = this.correct(this.config, config, defaultMap, parentPath, Collections.unmodifiableList(parentPath), listener, commentListener, false);
        } finally {
            ...
            return this.define(path, new ValueSpec(defaultSupplier, x -> x instanceof List && ((List<?>) x).stream().allMatch( elementValidator ), this.context, path) {
                ...
                public Object correct(Object value) {
                    if (value == null || !(value instanceof List) || ((List<?>)value).isEmpty()) {
                        // Forge Config API Port: replace with SLF4J logger
                        ForgeConfigAPIPort.LOGGER.debug("List on key {} is deemed to need correction. It is null, not a list, or an empty list. Modders, consider defineListAllowEmpty?", path.get(path.size() - 1));
                    ...
                    if (list.isEmpty()) {
                        // Forge Config API Port: replace with SLF4J logger
                        ForgeConfigAPIPort.LOGGER.debug("List on key {} is deemed to need correction. It failed validation.", path.get(path.size() - 1));
                        return this.getDefault();
                    }
                    return list;
                }
            ...
            return this.define(path, new ValueSpec(defaultSupplier, x -> x instanceof List && ((List<?>) x).stream().allMatch( elementValidator ), this.context, path) {
                ...
                public Object correct(Object value) {
                    if (value == null || !(value instanceof List)) {
                        // Forge Config API Port: replace with SLF4J logger
                        ForgeConfigAPIPort.LOGGER.debug("List on key {} is deemed to need correction, as it is null or not a list.", path.get(path.size() - 1));
                        return this.getDefault();
                    }
                    List<?> list = Lists.newArrayList((List<?>) value);
                    list.removeIf(elementValidator.negate());
    ...
    private static class BuilderContext
    {
        ...
        public String buildComment() { return this.buildComment(List.of("unknown", "unknown")); }
        public String buildComment(final List<String> path)
        {
            if (this.comment.stream().allMatch(String::isBlank))
            {
                if (!CommonAbstractions.INSTANCE.isDevelopmentEnvironment())
                    ForgeConfigAPIPort.LOGGER.warn("Detected a comment that is all whitespace for config option {}, which causes obscure bugs in NeoForge's config system and will cause a crash in the future. Please report this to the mod author.",
    ...
    public static class Range<V extends Comparable<? super V>> implements Predicate<Object>
    {
        ...
        public Class<? extends V> getClazz() { return this.clazz; }
        public V getMin() { return this.min; }
        public V getMax() { return this.max; }
        ...
        public boolean test(Object t)
        {
            if (this.isNumber(t))
            {
                ...
                {
                    // Forge Config API Port: replace with SLF4J logger
                    ForgeConfigAPIPort.LOGGER.debug("Range value {} is not within its bounds {}-{}", n.doubleValue(), ((Number) this.min).doubleValue(), ((Number) this.max).doubleValue());
                }
                return result;
            }
    ...
    public static class ValueSpec
    {
        ...
        public boolean needsWorldRestart() { return this.worldRestart; }
        public Class<?> getClazz(){ return this.clazz; }
        public boolean test(Object value) { return this.validator.test(value); }
        public Object correct(Object value) { return this.range == null ? this.getDefault() : this.range.correct(value, this.getDefault()); }

        public Object getDefault() { return this.supplier.get(); }
    }
    ...
    public static class ConfigValue<T> implements Supplier<T>
    {
        ...
        public T get()
        {
            ...
            if (USE_CACHES && this.cachedValue == null) this.cachedValue = this.getRaw(this.spec.childConfig, this.path, this.defaultSupplier);
            else if (!USE_CACHES)
