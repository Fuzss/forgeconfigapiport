...
public interface ForgeConfigPaths {
     ...
     * <p>Forge Config Api Port has a config setting (which is not present in Forge itself) to load server configs from the common directory instead. This method will always provide the directory that is currently set via the Forge Config Api Port config.
     *
     ...
     * <p>This path is configurable via the Forge Config Api Port config. The config is loaded during the <code>preLaunch</code> phase of Fabric Loader, it might therefore not be available in some early mixins.
