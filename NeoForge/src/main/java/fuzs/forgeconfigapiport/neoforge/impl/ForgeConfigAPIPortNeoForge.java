...
public class ForgeConfigAPIPortNeoForge {
    ...
    public static void onConstructMod(final FMLConstructModEvent evt) {
        if (CommonAbstractions.isDevelopmentEnvironment() && CommonAbstractions.includeTestConfigs()) {
            ModLoadingContext.get().registerConfig(ModConfig.Type.SERVER, new ModConfigSpec.Builder().comment("hello world").define("dummy_entry", true).next().build(), "forgeconfigapiport-server-neoforge.toml");
